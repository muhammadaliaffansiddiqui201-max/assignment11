...
// --- Realtime Orders Viewer (Collections Page) ---
...
const setOrdersEmptyState = (isEmpty, message) => {
    if (!ordersEmpty || !ordersList) return;
    ordersEmpty.style.display = isEmpty ? 'block' : 'none';
    if (message) ordersEmpty.innerText = message;
};
...
const renderOrders = (orders) => {
    ...
    if (!orders || orders.length === 0) {
        setOrdersEmptyState(true, 'No orders yet.');
        return;
    }
    ...
    orders.forEach((order) => {
        ...
        title.innerText = order.customer?.name || 'Customer';

        const meta = document.createElement('div');
        meta.className = 'order-meta';
        meta.innerText = `Order ID: ${order.id || 'N/A'} • ${formatOrderTime(order.createdAt)}`;

        const product = document.createElement('div');
        product.className = 'order-line';
        product.innerHTML = `Product: <span>${order.product?.name || 'N/A'}</span>`;

        const qty = document.createElement('div');
        qty.className = 'order-line';
        qty.innerHTML = `Qty: <span>${order.quantity || 1}</span>`;

        const price = document.createElement('div');
        price.className = 'order-line';
        price.innerHTML = `Total: <span>${order.totalText || 'Rs. 0'}</span>`;

        const payment = document.createElement('div');
        payment.className = 'order-line';
        payment.innerHTML = `Payment: <span>${order.paymentMethod || 'cod'}</span>`;

        const phone = document.createElement('div');
        phone.className = 'order-line';
        phone.innerHTML = `Phone: <span>${order.customer?.phone || 'N/A'}</span>`;

        const address = document.createElement('div');
        address.className = 'order-line';
        address.innerHTML = `Address: <span>${order.customer?.address || 'N/A'}</span>`;
...
const initRealtimeOrders = () => {
    if (!ordersList) return;
    if (!window.firebaseDb || typeof window.firebaseDb.ref !== 'function') {
        setOrdersEmptyState(true, 'Realtime orders not configured.');
        return;
    }
    ...
    try {
        ...
        ordersRef.on('value', (snapshot) => {
            ...
            const orders = entries.map(([id, order]) => ({
                id,
                ...order
            }));
            ...
            setOrdersEmptyState(true, 'Unable to load orders.');
        });
    } catch (err) {
        console.warn('Realtime orders init failed:', err);
        setOrdersEmptyState(true, 'Unable to load orders.');
    }
};
